...
describe('Menu Sort Business Flow', () => {
  ...
  it('Should handle complete sort menu flow including validation, edge cases, and response structure', async () => {
    ...
    // Sort menus
    const sortData = {
      list_menu: [{ id: menu3Id }, { id: menu1Id }, { id: menu2Id }],
    };
    ...
    const invalidSortData = {
      list_menu: [{ id: 999999 }, { id: menu1Id }],
    };
    ...
    const duplicateOrderData = {
      list_menu: [{ id: menu1Id }, { id: menu2Id }],
    };
    ...
    expect(duplicateOrderResponse.body.message).toBe(
      'Success to sort data menu.',
    );
    ...
    expect(negativeOrderResponse.body.message).toBe(
      'Success to sort data menu.',
    );
    ...
    const missingFieldsData = {
      list_menu: [{ id: menu1Id }, { order_number: 1 }],
    };
    ...
    expect(decimalOrderResponse.body.message).toBe(
      'Success to sort data menu.',
    );
    ...
    const concurrentSortPromises = Array(3)
      .fill(null)
      .map((_, index) => {
    ...
    const successfulSorts = concurrentSortResponses.filter(
    ...
    if (listResponse.body.data.length > 1) {
      for (let i = 0; i < listResponse.body.data.length - 1; i++) {
        expect(listResponse.body.data[i].order_number).toBeLessThanOrEqual(
          listResponse.body.data[i + 1].order_number,
        );
      }
    }
